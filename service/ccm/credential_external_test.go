...
func TestStatusStreamLoopUpdatesStateFromFrames(t *testing.T) {
	...
	burnFactor := c.state.remoteWeeklyBurnFactor
	...
	if burnFactor != 1.8 {
		t.Fatalf("expected remoteWeeklyBurnFactor=1.8, got %v", burnFactor)
	}
	if updated.IsZero() {
