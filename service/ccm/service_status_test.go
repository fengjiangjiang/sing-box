...
func TestHandleStatusStreamPushesUpdateWhenResetTimeElapses(t *testing.T) {
	...
	defer cancel()
	request := httptest.NewRequest(http.MethodGet, "/ccm/v1/status?watch=true", nil).WithContext(ctx)
...
func TestHandleStatusStreamSubscriptionResetsTimerWhenOnlyResetTimeChanges(t *testing.T) {
	...
	// Use asymmetric weights: credA (weight 1) vs credB (weight 9999).
	// Changing credA's fiveReset barely moves the weighted-average
	// FiveHourReset (< 1 epoch second), keeping toPayload() identical,
	// while earliestCredentialReset shifts dramatically.
	...
	go func() {
		...
		// Move credA's reset to ~100 ms from now. The weighted-average epoch
		// barely changes (delta < 0.06 s) so the payload stays equal,
		// but earliestCredentialReset moves from 10 min to ~100 ms.
	...
	body := recorder.Body.String()
	frames := strings.Split(strings.TrimSpace(body), "\n")
	// Without the fix: only 1 frame (timer stays at 10 min, never fires).
	// With the fix: timer resets to ~100 ms. When it fires, credA's 5h
	// utilization resets to 0, lowering the aggregate → new frame written.
	if len(frames) < 2 {
